 ...
 *
 *   STORY=pt-monkey-crocodile T=3600 node pipeline/still.js 07 08
 *
 * T is the timeline position in ms (default 4200) -- set it near a callout's `at` to see
 * the bubble. Frames land in build/stills/<film>-<id>.png.
 */
'use strict';
...
if (!process.env.STORY) throw new Error('set STORY=<film>, e.g. STORY=pt-monkey-crocodile');
const STORY = S.STORY;
const ids = process.argv.slice(2);
if (!ids.length) throw new Error('name at least one shot id, e.g. 07');

const FILM = S.FILM;
const OUT = path.join(S.REPO, 'build', 'stills');
